 ...
 * Write a program to help yourself quickly create large numbers for use in the code, given a limited number of plates to work with.You have numList,
 * a list containing some digits (0 to 9). Write a function solution(numList) which finds the largest number that can be made from some
 * or all of these digits and is divisible by 3. If it is not possible to make such a number, return 0 as the solution.
 * numList will contain anywhere from 1 to 9 digits.  The same digit may appear multiple times in the list, but each element in
